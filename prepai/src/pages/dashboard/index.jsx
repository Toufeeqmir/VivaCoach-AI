...
import { buildReadiness, getWeekActivity } from "./DashboardSidebar";
...
const Dashboard = () => {
  ...
  const totalMinutes = useMemo(() => Math.floor((report?.totalDurationSeconds || 0) / 60), [report]);
  const stats = useMemo(() => {
  ...
  }, [interviews.length, report, totalMinutes]);

  const firstName = (user?.name || "there").split(" ")[0];
  const hasActivity = stats.some((item) => Number.parseInt(item.value, 10) > 0);
  const readiness = useMemo(() => buildReadiness(interviews), [interviews]);
  const week = useMemo(() => getWeekActivity(interviews), [interviews]);
